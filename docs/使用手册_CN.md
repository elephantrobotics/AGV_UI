...
![Software Page](chinese-images/software_page.png)
...
![Language Choose](chinese-images/language_choose.png "Choose Language")

## 2. 雷达功能

![Radar Open](chinese-images/radar_open.png "Open Radar")

![Radar Close](chinese-images/radar_close.png "Close Radar")

1. 点击按钮，打开雷达。按钮变成红色，文字显示为“关闭”。
2. myAgv的雷达开始转动。
3. 桌面出现新的终端如图示，说明雷达打开成功。

![Radar Open](chinese-images/radar_open_terminal.png "Open Radar")
...
![Basic Control](chinese-images/basic_control.png "Basic Control")
...
![Keyboard terminal](chinese-images/keyboard_terminal.png "Keyboard terminal")  
...
![JoyStick Alphabet](chinese-images/joystick_alphabet_terminal.png  "JoyStick Alphabet")

> 如图所示，1~4控制小车前进后退和左右运动，5控制小车逆时针旋转，6控制小车顺时针旋转，7为停止按钮。

![JoyStick Alphabet](chinese-images/joystick_alphabet.jpg)
...
![JoyStick Number](chinese-images/joystick_number.png "JoyStick Number")

## 4. 地图导航

![Map Navigation](chinese-images/map_navigation.png "Map and navigation")
...
![Build Map](chinese-images/mapping.png  "Build Map")
...
1. 桌面显示rviz界面
2. 选中已打开的键盘终端，使用键盘控制小车，rviz空间会随着小车的移动将地图构建出来。轨迹如图所示：  ![Gmapping Map](img_depict/gmapping_rviz.png  "Gmapping Map")

### 5.2 Rtabmap

点击下拉框选择`Rtabmap`建图方式，点击“打开建图”按钮开始建图。

1. 桌面打开新终端，若终端内不断滚动输出数据，则表示小车cartographer建图文件成功打开，终端内显示状态如下：   ![Cartographer Map](img_depict/cartographer_terminal.png)
2. 代码运行成功后会打开rviz，这时在rviz中便会显示地图和雷达信息，红色箭头为小车的朝向。界面如图所示。   ![Cartographer Map](img_depict/cartograph_rviz.jpg)
3. 选中已打开的键盘终端，使用键盘控制小车，rviz空间会随着小车的移动将地图构建出来。轨迹如图所示：  ![Cartographer Track](img_depict/catograph_rviz2.jpg)

**限制:** 开启建图后导航不可使用；若需使用导航功能请先关闭建图。

### 5.3 保存建图

![Save Map](img_depict/save_map_cn.png "Save Map")
...
![Map Navigation](img_depict/navagation_cn.png "Map Navigation")
...
![LED Light](chinese-images/led_light.png "LED Light")

> 通过圆盘选择灯光颜色，拖动滑动条改变灯光颜色的亮度。图示右侧则显示对应的HEX和RGB值。雷达启动时不可以控制LED灯。

## 7. 测试功能

![Testing Function](chinese-images/functional_testing.png "Testing Function")
...
![Log Area](chinese-images/log_area.png "Log Area")

以上所有操作均会显示在日志区域。点击图示右上角“清除按钮”清空日志区域当前内容。

## 8. 状态监测

![Status Detecting](chinese-images/condition_monitoring.png "Status Information")
